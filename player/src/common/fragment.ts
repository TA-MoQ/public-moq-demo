...
export class FragmentedMessageHandler {
  ...
  private storeFragment(fragment: MessageFragment) {
    if (!this.fragmentBuffers.has(fragment.chunkID)) {
      this.fragmentBuffers.set(
        fragment.chunkID,
        new Array(fragment.fragmentTotal).fill(null)
      );
    }
    ...
    if (fragmentBuffer) {
      ...
      if (fragmentBuffer.every((element) => element !== null)) {
        const totalLength = fragmentBuffer.reduce(
  ...
  private enqueueChunk(
    ...
    if (boxType === 'finw') {
      const dv = new DataView(
        chunk.slice(8).buffer,
        chunk.slice(8).byteOffset,
        chunk.slice(8).byteLength
      );
