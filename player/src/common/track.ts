...
export class Track {
  ...
  add(segment: Segment) : number {
    ...
    console.debug(`[NEW ${this.type.toUpperCase()} SEGMENT] ${segment.timestamp}`, segment)

    if (this.segments.length && this.segments[0].timestamp > segment.timestamp){
      return this.segments.length
    }
    else if (!this.segments.length || this.segments[this.segments.length - 1].timestamp < segment.timestamp){
      this.segments.push(segment)

    }
    else if (this.segments[this.segments.length - 1].timestamp - segment.timestamp < 4){
      this.segments.push(this.segments[this.segments.length - 1]);
      this.segments[this.segments.length - 2] = segment;
    }
    console.log(`[CURR ${this.type.toUpperCase()} SEGMENTS] length=${this.segments.length}`, this.segments.pop())
    return this.segments.length
