...
export class Player {
  ...
  start = async () => {
    ...
    this.started = true;
    this.paused =
      (this.continueStreamingRef.dataset.status || 'streaming') === 'paused';
    ...
    this.continueStreamingRef.addEventListener(
      'click',
      this.continueStreamingClicked
    );
    ...
    if (this.activeBWTestInterval > 0) {
      setTimeout(() => {
        this.activeBWTestTimer = setInterval(
          () => this.startActiveBWTest(),
  ...
  pauseOrResume = (pause?: boolean) => {
    ...
    if (sendMessage) {
      this.continueStreamingRef.dataset.status = this.paused
        ? 'paused'
        : 'streaming';
  ...
  throttleOnChange = () => {
    ...
    if (this.throttleDDLRef.options.selectedIndex > 0) {
      rate = parseInt(
        this.throttleDDLRef.options[this.throttleDDLRef.options.selectedIndex]
          .value
      );
    }
        ...
        fetch(url, { cache: 'no-store' })
          ...
          .then((resp) => {
            ...
            if (!size) {
              ...
              this.activeBWTestResult = Math.round(
                (size * 8 * 1000) / duration
              );
              ...
              this.activeBWResetTimer = setTimeout(() => {
                this.activeBWTestResult = 0;
              }, 1000);
  ...
  async handleSegment(
    ...
    // TODO: UNCOMMENT LOG
    console.log(
      'msg: %o tcRate: %d serverBandwidth: %d',
      msg,
      this.tcRate,
    ...
    var mediaBufferSize = track.add(segment);
    console.log("[INFO] Media Buffer size : ", mediaBufferSize, " segments")
    ...
    while (true) {
      ...
      if (isVideoSegment) {
        if (boxType === 'moof') {
          ...
          const stat = [
            ...
            chunkDownloadDuration,
            lastMoofDownloadDuration,
            chunkDownloadDuration > 0
              ? (chunkSize * 8 * 1000) / chunkDownloadDuration
              : 0,
            chunkLatency,
            msg.timestamp,
          ];
          ...
          if (chunkLatencies.length > 1) {
            ...
            let latency =
              chunkLatencies[lastData] - chunkLatencies[lastData - 1];
      ...
      const segmentTPut = this.computeSegmentTPut(
        this.totalSizeProcessed,
        performance.now()
      );
      if (segmentTPut > 0) {
    ...
    if (msg.init != '4') {
      avgSegmentLatency =
        this.calculateAverageChunkLatency(chunkLatencies).toFixed(2);
      avgSegmentLatency2 =
        this.calculateAverageChunkLatency2(chunkLatencies).toFixed(2);
      // console.log(`
      // 			=====================================================
    ...
    if (isVideoSegment) {
      ...
      this.logFunc(
        'availability time (server): ' + new Date(msg.at).toISOString()
      );
      if (msg.init != '4') {
    ...
    if (this.isAuto) {
      ...
      if (
        Number(serverBandwidthInMegabits) >= 4 &&
        Number(avgSegmentLatency) > 100
      ) {
        ...
        Number(serverBandwidthInMegabits) < 4 &&
        Number(serverBandwidthInMegabits) >= 2.6 &&
        Number(avgSegmentLatency) > 100
      ) {
        this.changeQuicType(2);
      } else if (
        ...
        Number(serverBandwidthInMegabits) < 2.6 &&
        Number(serverBandwidthInMegabits) >= 1.3 &&
        Number(avgSegmentLatency) > 100
      ) {
        this.changeQuicType(2);
      } else if (
        ...
        Number(serverBandwidthInMegabits) < 1.3 &&
        Number(serverBandwidthInMegabits) >= 0.365 &&
        Number(avgSegmentLatency) > 100
      ) {
        this.changeQuicType(2);
      } else if (
        ...
        Number(serverBandwidthInMegabits) < 0.365 &&
        Number(serverBandwidthInMegabits) >= 0 &&
        Number(avgSegmentLatency) > 100
      ) {
        this.changeQuicType(2);
      } else if (
  ...
  logChunkStats = (filteredChunkStats: any[]) => {
    ...
    if (this.getSWMACalculationType() === 'window') {
      ...
      this.logFunc(
        'calculation interval: ' + this.getSWMACalculationInterval()
      );
    }
    ...
    this.logFunc(
      'number of discarded chunks: ' +
        (this.chunkStats.length - filteredChunkStats.length)
    );
    this.logFunc('');
    this.logFunc(
      '#\tChunk Size(byte)\tMDat Download Duration(ms)\tMoof Download Duration\tDownload Rate\tAvailability Offset (ms)\tSegment TS'
    );
  ...
  filterStats = (
    ...
    // TODO: UNCOMMENT LOG
    console.log(
      'computeTPut | chunk count: %d thresholdType: %s threshold: %d',
  ...
  calculateAverageChunkLatency(arrivalTimes: number[]): number {
    ...
    const totalLatency = chunkLatencies.reduce(
  ...
  computeSegmentTPut = (totalSize: number, timestart: number) => {
    ...
    return (totalSize * 8 * 1000) / (timestart - (this.timeRef ?? 0));
  };

  updateStats = () => {
    const audioFiller = this.statsRef.querySelector(
      '.audio.buffer .fill'
    ) as HTMLElement;
    const audioBufferDurationEl = this.statsRef.querySelector(
      '.audio.label>.seconds'
    ) as HTMLElement;
    if (audioFiller) {
    ...
    const videoFiller = this.statsRef.querySelector(
      '.video.buffer .fill'
    ) as HTMLElement;
    const videoBufferDurationEl = this.statsRef.querySelector(
      '.video.label>.seconds'
    ) as HTMLElement;
    if (videoFiller) {
    ...
    const bw_swma_threshold = document.querySelector(
      '#stats .swma_threshold'
    ) as HTMLDivElement;
    const chunk_throughput = document.querySelector(
      '#stats .chunk_throughput'
    ) as HTMLDivElement;
    const chunk_latency = document.querySelector(
      '#stats .chunk_latency'
    ) as HTMLDivElement;
    const chunk_volume = document.querySelector(
      '#stats .total_chunk_volume'
    ) as HTMLDivElement;
    const elapsed_time = document.querySelector(
      '#stats .elapsed_time'
    ) as HTMLDivElement;
    const bw_active_bw = document.querySelector(
      '#stats .active_bw'
    ) as HTMLDivElement;
    const quic_type = document.querySelector(
      '#stats .quic_type'
    ) as HTMLDivElement;
    const swma_latency = document.querySelector(
      '#stats .swma_chunk_latency'
    ) as HTMLDivElement;
    ...
    if (bw) {
      ...
      chunk_throughput.innerText = formatBits(
        this.throughputs.get('chunk') || 0,
        1
      ).toString();
      chunk_volume.innerText = formatBits(
        this.totalSizeProcessed * 8,
        1
      ).toString();
      elapsed_time.innerText = (
        (performance.now() - (this.timeRef ?? 0 )) /
        1000
      ).toString();
      chunk_latency.innerText =
        this.throughputs.get('avgSegmentLatency')?.toString() || '0';
      quic_type.innerText = this.currCategory;
      swma_latency.innerText =
        this.throughputs.get('SWMALatency')?.toString() || '0';
  ...
  visualizeBuffer(
    ...
    for (let i = 0; i < ranges.length; i += 1) {
      ...
      bufferFiller.setAttribute(
        'style',
        'left: ' +
          (100 * Math.max(start, 0)) / max +
          '%; right: ' +
          (100 - (100 * Math.min(end, max)) / max) +
          '%'
      );
      index += 1;
