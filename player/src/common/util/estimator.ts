...
const computeByTestId = async (testId: string) => {
  ...
  if (logs?.length > 0) {
    ...
    logs.forEach((item, i) => {
      ...
      if (
        totalChunkCount >= getSWMAWindowSize() &&
        totalChunkCount % getSWMACalculationInterval() === 0
      ) {
        let stats = chunkStats.slice(-getSWMAWindowSize());
        let filteredStats = filterStats(
          stats,
          getSWMAThreshold(),
        ...
        tputs.push([
          ...
          !item.msg_tc_rate
            ? 0
            : Math.round(Math.abs(tput - item.msg_tc_rate * 1000)), // abs diff
          !item.msg_tc_rate
            ? 0
            : Math.round(
                (Math.abs(tput - item.msg_tc_rate * 1000) /
                  (item.msg_tc_rate * 1000)) *
                  100
              ), // abs diff percent
          !item.msg_tc_rate
            ? 0
            : Math.round(Math.abs(tput2 - item.msg_tc_rate * 1000)), // abs diff
          !item.msg_tc_rate
            ? 0
            : Math.round(
                (Math.abs(tput2 - item.msg_tc_rate * 1000) /
                  (item.msg_tc_rate * 1000)) *
                  100
              ), // abs diff percent
        ]);
    ...
    const totalAbsDiffPercentage = tputs.reduce(
    ...
    const totalAbsDiffPercentage_2 = tputs.reduce(
      (prev: number, current: any) => {
    ...
    const meanAbsDiffPercentage = Math.round(
      totalAbsDiffPercentage / tputs.length
    );
    const meanAbsDiff_2 = Math.round(totalAbsDiff_2 / tputs.length);
    const meanAbsDiffPercentage_2 = Math.round(
      totalAbsDiffPercentage_2 / tputs.length
    );
    ...
    console.log(
      'meanAbsDiff: %f meanAbsDiffPercentage: %f meanAbsDiff_2: %f meanAbsDiffPercentage_2: %f total: %d',
      meanAbsDiff,
      meanAbsDiffPercentage,
      meanAbsDiff_2,
      meanAbsDiffPercentage_2,
      tputs.length
    );
  }
};
...
const filterStats = (
  ...
  if (lastTPut) {
    ...
    filteredStats = filteredStats.filter((m) => {
      ...
      if (
        lastTPut < magicalNumber * chunkTPut &&
        lastTPut * magicalNumber > chunkTPut
      ) {
        return true;
      } else {
