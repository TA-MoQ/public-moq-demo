    ...
    try {
      ...
      DBOpenRequest.onupgradeneeded = (event: any) => {
        ...
        try {
          ...
          logStore = db.createObjectStore('test_logs', {
            keyPath: 'key',
            autoIncrement: true,
          });
          testStore = db.createObjectStore('tests', { keyPath: 'testId' });
          resultStore = db.createObjectStore('test_results', {
            keyPath: 'key',
            autoIncrement: true,
          });
          segmentLogStore = db.createObjectStore('segment_logs', {
            keyPath: 'key',
            autoIncrement: true,
          });
...
const getEntriesByTestId = async (
  storeName: string,
  testId: string
): Promise<any[]> => {
